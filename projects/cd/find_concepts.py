...
class CandidateSelection(BaseModel):
    ...
    candidate_ids: List[int] = Field(max_length=12, description="Ordered list of candidate concept IDs")
...
candidate_aggregator_agent = Agent(
    "openai:gpt-5-mini",
...
def _unique_sorted_ints(values: List[Any], limit: Optional[int] = None) -> List[int]:
...
def _minify_concept(concept_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Extract concept ID
    cid = _coerce_int(
        concept_data.get("concept_id") or
        concept_data.get("conceptId") or
        concept_data.get("id")
    )
    
    # Extract minimal details
    minimal_details: Dict[str, Any] = {}
    details_source = concept_data.get("details") or concept_data.get("summary", {}).get("details", {})
...
def _included_from_details(concept_id: Optional[int], details: Dict[str, Any]) -> Optional[Dict[str, Any]]:
...
def _try_short_circuit_resolution(
    ...
    for concept in concepts:
        ...
        domain = str(details.get("domainId", ""))
        cls = str(details.get("conceptClassId", ""))
        name = details.get("name")
        syns = details.get("synonyms") if isinstance(details.get("synonyms"), list) else []
        ...
        if std and (exact_match or strong_token_match):
            ...
            elif vocab in {"RXNORM", "RXNORM EXTENSION"} and cls in {"Ingredient", "Precise Ingredient"}:
                return {
                    "concept_id": cid,
...
def _update_queue_from_batch(
    ...
    queue_state.pending = [item for item in queue_state.pending if item.concept_id not in processed_ids]
    ...
    for i, decision in enumerate(decisions):
        if decision.is_standard and decision.is_correct_for_term:
            ...
            if all((_coerce_int(x.get("concept_id")) != decision.concept_id) for x in queue_state.accepted_concepts):
    ...
    for i, decision in enumerate(decisions):
        ...
        for suggested_id in decision.suggested_new_candidates:
            if suggested_id not in queue_state.visited and suggested_id not in [item.concept_id for item in queue_state.pending]:
                child_depth = depth + 1
                if child_depth <= queue_state.max_depth:
                    queue_state.pending.append(QueueItem(concept_id=suggested_id, depth=child_depth))
...
def run_intelligent_concept_discovery(
    ...
    for concept_set in plan.concept_sets:
        ...
        for query in concept_set.queries[:MAX_QUERIES_PER_SET]:
            ...
            try:
                ...
                if search_result.get("success") and search_result.get("candidates"):
                    ...
                    print(f"      ⚠️  No candidates found: {search_result.get('error', 'Unknown error')}")
        ...
        print(f"  🤖 LLM candidate selection...")
        try:
            ...
            selection = CandidateSelection(
                ...
                candidate_ids=[c.get("concept_id") for c in all_search_results[:5] if c.get("concept_id")]
            )
        ...
        while not queue_state.resolved and queue_state.visit_count < max_visits and queue_state.pending:
            ...
            for cid in ids:
                ...
                try:
                    ...
                    concept_data = {
                        "concept_id": cid,
                        "details": details_result.get("concepts", [{}])[0] if details_result.get("success") else {},
                        "relationships": relationships_result.get("relationships", []) if relationships_result.get("success") else []
                    }
            ...
            if short_circuit:
                ...
                if sc_id is not None:
                    ...
                    matched = next((c for c in concepts if _coerce_int(c.get("concept_id")) == sc_id), None)
                    if matched:
                        inc = _included_from_details(sc_id, matched.get("details", {}))
                        if inc and all((_coerce_int(x.get("concept_id")) != sc_id) for x in queue_state.accepted_concepts):
                            queue_state.accepted_concepts.append(inc)
...
"""
            ...
            for decision in decisions:
                print(f"      - {decision.concept_id}: {'✅' if decision.is_standard and decision.is_correct_for_term else '❌'} {decision.reasoning[:100]}...")
            ...
            if len(queue_state.accepted_concepts) >= MAX_ACCEPTED_PER_SET:
                print(f"    ✅ Collected {len(queue_state.accepted_concepts)} accepted anchors; stopping exploration")
                queue_state.stop_reason = "enough_matches"
                break
            ...
            if queue_state.pending:
                ...
                if queue_state.last_head_id == head_id:
                    ...
                    if queue_state.stagnation_count >= 3:
                        print(f"    ⚠️  Stagnation detected, exiting")
        ...
        print(f"  ✅ Resolution: {outcome.status} ({outcome.reason}) after {outcome.visit_count} visits")
