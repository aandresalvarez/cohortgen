...
- Searches ATHENA for the best matching standard concepts per set (with optional domain/vocabulary hints).
- Produces a final JSON spec: `{ concept_sets: [{ name, concept_ids, include_descendants, standard_only }], assumptions: [] }` suitable for ATLAS concept set creation.
...
You will be prompted to paste a cohort definition. The pipeline will decompose concepts, search ATHENA, and output a final JSON concept set spec.

## Notes

- Skills live under `skills/` (`skills/athena_tools.py` provides the ATHENA integration).
