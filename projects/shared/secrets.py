...
class SecretConfig:
    ...
    required: bool = True
    description: str = ""
    env_names: Optional[List[str]] = None  # Alternative environment variable names
...
def get_secret(secret_name: str, required: bool = True) -> Optional[str]:
    ...
    env_names_raw = secret_config.env_names if secret_config else [secret_name]
    env_names_list: List[str] = env_names_raw if isinstance(env_names_raw, list) else [secret_name]
...
def main():
    ...
    required_missing = any(not status.get(s.name, False) for s in SECRETS if s.required)
