...
def test_clarification_decision_model():
    ...
    ask_decision = ClarificationDecision(
        action="ask", question="What is the index event?", cohort_components=CohortDefinition()
    )
    ...
    finish_decision = ClarificationDecision(
        action="finish", question="", cohort_components=CohortDefinition(index_event="flu test")
    )
...
def test_has_essential_components():
    ...
    # Complete cohort
    complete = CohortDefinition(index_event="diabetes diagnosis", inclusion_criteria=["age 18+"])

    # Incomplete cohort (no index event)
    incomplete = CohortDefinition(inclusion_criteria=["age 18+"])
...
def test_demographics_structure():
    ...
    cohort = CohortDefinition(demographics={"age": "20-30", "gender": "male", "ethnicity": "any"})
...
def test_multiple_exclusion_criteria():
    ...
    cohort = CohortDefinition(
        exclusion_criteria=["type 1 diabetes", "pregnancy", "prior insulin use"]
    )
...
def test_cohort_definition_to_dict():
    ...
    cohort = CohortDefinition(index_event="flu test", demographics={"age": "20-30"})
...
def test_empty_demographics():
    ...
    cohort = CohortDefinition(index_event="diabetes diagnosis", demographics={})
...
def test_empty_criteria_lists():
    ...
    cohort = CohortDefinition(index_event="test", inclusion_criteria=[], exclusion_criteria=[])
...
def test_finish_with_empty_question():
    ...
    decision = ClarificationDecision(
        action="finish", question="", cohort_components=CohortDefinition(index_event="test")
    )
...
def test_complete_cohort_definition():
    ...
    cohort = CohortDefinition(
        ...
        exclusion_criteria=["pregnancy at index", "hospice care in prior 90 days"],
...
def test_minimal_cohort_definition():
    ...
    cohort = CohortDefinition(index_event="positive COVID-19 test")
