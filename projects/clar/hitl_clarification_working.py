...
demographics_extractor = Agent(  # type: ignore[call-overload]
    "openai:gpt-5-mini",
...
def extract_demographics(cohort_description: str) -> tuple[dict[str, str], Optional[str]]:
    """
...
def run_clarification_loop(
    initial_cohort_description: str, max_questions: int = 10
) -> CohortDefinition:
    """
    ...
    if demographics:
        cohort_def.demographics = cast(Dict[str, Optional[Union[str, bool]]], demographics)
    ...
    for iteration in range(1, max_questions + 1):
        ...
        if decision.cohort_components:
            ...
            extracted_demographics = (
                cohort_def.demographics.copy() if cohort_def.demographics else {}
            )
            extracted_time_period = (
                cohort_def.observation_window if cohort_def.observation_window else ""
            )
        ...
        if decision.question:
            ...
            # Track conversation
            conversation_history.append({"question": decision.question, "answer": user_input})
