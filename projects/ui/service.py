...
class CohortService:
    ...
    def start_run(self, run_id: str, stages: list[int] | None = None) -> None:
        """
    ...
    def _execute_stage1(self, run: CohortRun) -> None:
        ...
        try:
            ...
            def mock_input(prompt=""):
                ...
                log_print("👤 You: [auto-proceeding with defaults]")
            ...
            try:
                ...
                # Log start
                log_print("=" * 70)
                log_print("Stage 1: Clinical Clarification")
            ...
            stage_result.metrics.duration_seconds = (datetime.now() - start_time).total_seconds()
    ...
    def _execute_stage2(self, run: CohortRun) -> None:
        ...
        try:
            ...
            os.environ["MAX_CONCEPT_SETS"] = str(run.user_inputs.max_concept_sets)
            os.environ["MAX_QUERIES_PER_SET"] = str(run.user_inputs.max_queries_per_set)
            os.environ["SEARCH_TOP_K"] = str(run.user_inputs.search_top_k)
            os.environ["PER_SET_TIME_LIMIT_SEC"] = str(run.user_inputs.per_set_time_limit_sec)
            os.environ["MAX_ACCEPTED_PER_SET"] = str(run.user_inputs.max_accepted_per_set)
            ...
            stage_result.metrics.concepts_found = sum(
                len(cs.get("included_concepts", [])) for cs in concept_sets.get("concept_sets", [])
            )
    ...
    def _execute_stage3(self, run: CohortRun) -> None:
        ...
        try:
            ...
            try:
                ...
                from create_bigquery_sql import CohortInput, run_bigquery_sql_generation
    ...
    def _execute_stage4(self, run: CohortRun) -> None:
        ...
        try:
            ...
            modules_to_clear = [mod for mod in list(sys.modules.keys()) if "tools" in mod]
            for mod in modules_to_clear:
                if mod in sys.modules:
                    del sys.modules[mod]
            ...
            try:
                ...
                from run_stats import build_analytics_queries, extract_concept_ids
                ...
                try:
                    ...
                    dataset = run.user_inputs.omop_dataset_id or os.getenv(
                        "OMOP_DATASET_ID", "bigquery-public-data.cms_synthetic_patient_data_omop"
                    )
                    location = run.user_inputs.bigquery_location or "US"
                    ...
                    if not project_id:
                        ...
                        if run.stage2_path and os.path.exists(run.stage2_path):
                        ...
                        esrd_ids = (
                            extract_concept_ids(input_data, ["end-stage renal disease", "esrd"])
                            or []
                        )
                        if esrd_ids:
                            ...
                            log_print(
                                "No ESRD concept IDs found; age/index-year analytics will be skipped"
                            )
                        log_print("")
                        ...
                        queries = build_analytics_queries(
                            sql_for_analytics, project_id, dataset, esrd_ids
                        )
                ...
                # Clear imported modules from this stage
                modules_to_clear = [mod for mod in sys.modules.keys() if mod == "run_stats"]
                for mod in modules_to_clear:
                    del sys.modules[mod]
    ...
    def _format_cohort_for_stage2(self, stage1_output: dict[str, Any]) -> str:
        ...
        if stage1_output.get("demographics"):
            demo_str = ", ".join(f"{k}: {v}" for k, v in stage1_output["demographics"].items())
            parts.append(f"Demographics: {demo_str}")

        if stage1_output.get("inclusion_criteria"):
            parts.append(f"Inclusion Criteria: {', '.join(stage1_output['inclusion_criteria'])}")

        if stage1_output.get("exclusion_criteria"):
            parts.append(f"Exclusion Criteria: {', '.join(stage1_output['exclusion_criteria'])}")

        if stage1_output.get("observation_window"):
            parts.append(f"Observation Window: {stage1_output['observation_window']}")

        if stage1_output.get("prior_observation"):
            parts.append(f"Prior Observation: {stage1_output['prior_observation']}")
    ...
    def start_interactive_clarification(self, run_id: str) -> tuple[str, bool]:
        ...
        session = InteractiveClarificationSession(
            run_id=run_id, initial_description=run.user_inputs.cohort_description
        )
    ...
    def send_clarification_message(
        ...
        if not session:
            ...
            if run and run.clarification_session and not run.clarification_session.is_complete:
                ...
                session.question_count = len(
                    [msg for msg in session.conversation_history if msg["role"] == "assistant"]
                )
