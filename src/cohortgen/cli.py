...
def _run(cmd: list[str], cwd: Path | None = None) -> int:
    ...
    if cwd is not None:
        ...
        env["PYTHONPATH"] = str(cwd) if not pp else f"{cwd}{os.pathsep}{pp}"
...
def cmd_init(ns: argparse.Namespace) -> int:
    ...
    rc = _run([sys.executable, "-m", "flujo.cli.main", "init"], cwd=target)
    return rc
...
def cmd_run(ns: argparse.Namespace) -> int:
    ...
    extra = ns.args or []
    cmd = [sys.executable, "-m", "flujo.cli.main", "run", *extra]
